...
// New tests for operator overloading
TEST_CASE("ComplexDomino Operator Overloading", "[ComplexDomino][Operators]") {
    SECTION("Operator += for adding domino") {
        ComplexDomino comp;
        SimpleDomino dom(3, 5, false);
    ...
    SECTION("Operator ! for sorting") {
        ComplexDomino comp;
        comp += SimpleDomino(6, 6, false);
        comp += SimpleDomino(1, 1, false);
        comp += SimpleDomino(3, 4, false);

        !comp;

        REQUIRE(comp[0].getVal1() + comp[0].getVal2() == 2);
        REQUIRE(comp[1].getVal1() + comp[1].getVal2() == 7);
        REQUIRE(comp[2].getVal1() + comp[2].getVal2() == 12);
    }

    SECTION("Operator () for subsequence extraction") {
        ComplexDomino comp;
        comp += SimpleDomino(3, 5, false);
        comp += SimpleDomino(1, 3, false);
        comp += SimpleDomino(2, 4, false);
        ...
        ComplexDomino subsequence = comp(3);
    ...
    SECTION("Operator [] for element access") {
        ComplexDomino comp;
        SimpleDomino dom(2, 4, false);
        comp += dom;

        REQUIRE(comp[0] == dom);
        comp[0].setVal1(5);
        REQUIRE(comp[0].getVal1() == 5);
    }

    SECTION("Operator [] const for element access") {
        ComplexDomino comp;
        SimpleDomino dom(2, 4, false);
        comp += dom;

        const ComplexDomino& constComp = comp;
        REQUIRE(constComp[0] == dom);
        REQUIRE(constComp[0].getVal1() == 2);
    }

    SECTION("Operator chaining for ComplexDomino") {
        ComplexDomino comp;

        // Правильная цепочка операторов для ComplexDomino
        comp += SimpleDomino(1, 2, false);
        comp += SimpleDomino(3, 4, false);

        REQUIRE(comp.getSize() == 2);

        !comp; // Sort

        REQUIRE(comp[0].getVal1() + comp[0].getVal2() == 3);
        REQUIRE(comp[1].getVal1() + comp[1].getVal2() == 7);
    }
}

TEST_CASE("ComplexDomino Advanced Operations", "[ComplexDomino][Advanced]") {
    SECTION("Multiple subsequence extractions") {
        ComplexDomino comp;
        comp += SimpleDomino(1, 2, false);
        comp += SimpleDomino(1, 3, false);
        comp += SimpleDomino(2, 4, false);
        comp += SimpleDomino(1, 5, false);
        comp += SimpleDomino(3, 6, false);

        ComplexDomino ones = comp(1);
        REQUIRE(ones.getSize() == 3);
        REQUIRE(comp.getSize() == 2);

        ComplexDomino twos = comp(2);
        REQUIRE(twos.getSize() == 1);
        REQUIRE(comp.getSize() == 1);
    }
    ...
    SECTION("Empty operations") {
        ...
        // Operations on empty collection should not crash
        comp.removeRandom();
        comp.remove(1, 2);
        comp.removeByIndex(0);
        ...
        // Sorting empty collection
        !comp;
        REQUIRE(comp.getSize() == 0);

        // Subsequence from empty collection
        ComplexDomino emptySeq = comp(1);
        REQUIRE(emptySeq.getSize() == 0);
    }

    SECTION("Large collection operations") {
        ComplexDomino comp(true, 100, -1);
        REQUIRE(comp.getSize() == 100);

        // Sort large collection
        !comp;

        // Verify sorted order
        for (size_t i = 0; i < comp.getSize() - 1; ++i) {
            REQUIRE(comp[i].getVal1() + comp[i].getVal2() <=
                comp[i + 1].getVal1() + comp[i + 1].getVal2());
        }
    }
}